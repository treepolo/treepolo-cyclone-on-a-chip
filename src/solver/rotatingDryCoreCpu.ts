...
export interface RotatingStepOptions{heldSuarez?:boolean;momentumTransport?:boolean;coriolis?:boolean}
export class RotatingDryCoreCpu{
  ...
  step(s:DryState,dt:number,opt:RotatingStepOptions={}):StepDiagnostics{const cor=opt.coriolis!==false,mom=opt.momentumTransport!==false,hs=opt.heldSuarez!==false;if(cor)applyTraditionalCoriolis(this.h,this.rotation,s,.5*dt);const d=this.dry.step(s,dt);if(mom){if(!this.dry.lastTransport)throw new Error('missing transport snapshot');this.advectMomentum(s,dt,this.dry.lastTransport)}if(hs)applyHeldSuarezForcing(this.h,this.v,s,dt);if(cor)applyTraditionalCoriolis(this.h,this.rotation,s,.5*dt);return d}
