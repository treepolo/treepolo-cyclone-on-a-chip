...
export interface Stage4FrozenRhs {
  rhoD:Float64Array;rhoThetaM:Float64Array;uEdge:Float64Array;wInterface:Float64Array;pressure:Float64Array;dpdRhoTheta:Float64Array;
}
...
/**
 * Frozen RHS at one RK predictor. The slow evaluator supplies advection,
 * Coriolis and forcing. Predictor-value fast terms are then added once:
 * horizontal pressure acceleration, vertical reference-state scalar flux
 * divergence, and well-balanced vertical pressure+buoyancy acceleration.
 * Predictor-relative perturbations of those fast terms are integrated on the
 * acoustic small steps.
 */
export function computeStage4FrozenRhs(h:CubedSphereGrid,v:VerticalGrid,ref:ReferenceAtmosphere,predictor:DryState,options:Stage4SlowOptions={},rotation?:RotationGeometry):Stage4FrozenRhs {
  ...
  // Predictor horizontal pressure acceleration.
  ...
  // Predictor reference vertical scalar flux is the zero-perturbation fast
  // continuity term. Slow vertical continuity contains Fpert only.
  for(let c=0;c<h.cellCount;c++)for(let k=0;k<nz;k++){
    ...
    rho[q]=rho[q]!-(ref.rhoInterface[k+1]!*wt-ref.rhoInterface[k]!*wb)/dz;
    x[q]=x[q]!-(ref.rhoThetaInterface[k+1]!*wt-ref.rhoThetaInterface[k]!*wb)/dz;
  }
...
export function advanceStage4AcousticSmallStep(h:CubedSphereGrid,v:VerticalGrid,ref:ReferenceAtmosphere,predictor:DryState,acoustic:DryState,frozen:Stage4FrozenRhs,dt:number,options:Stage4Rk3SplitOptions={},rayleighRates?:ArrayLike<number>):void {
  ...
  // Forward horizontal momentum perturbation.
  for(let e=0;e<h.edgeCount;e++){
    const ge=h.edges[e]!,dist=Math.max(ge.centerDistanceAngle*R,1);
    for(let k=0;k<nz;k++){
      const l=cell3DIndex(ge.leftCell,k,nz),r=cell3DIndex(ge.rightCell,k,nz),q=edge3DIndex(e,k,nz),drL=acoustic.rhoD[l]!-predictor.rhoD[l]!,drR=acoustic.rhoD[r]!-predictor.rhoD[r]!,dxL=acoustic.rhoThetaM[l]!-predictor.rhoThetaM[l]!,dxR=acoustic.rhoThetaM[r]!-predictor.rhoThetaM[r]!;
      const rhoAvg=Math.max(.5*(predictor.rhoD[l]!+predictor.rhoD[r]!),1e-12),dRhoAvg=.5*(drL+drR),dpPred=frozen.pressure[r]!-frozen.pressure[l]!,dDp=frozen.dpdRhoTheta[r]!*dxR-frozen.dpdRhoTheta[l]!*dxL,linearPressure=-dDp/(rhoAvg*dist)+dpPred*dRhoAvg/(rhoAvg*rhoAvg*dist);
      acoustic.uEdge[q]=acoustic.uEdge[q]!+dt*(frozen.uEdge[q]!+linearPressure);
    }
  }

  // Backward horizontal scalar correction: only the predictor-relative flux
  // increment is added here; predictor full horizontal flux is in frozen RHS.
  ...
  for(let e=0;e<h.edgeCount;e++){
    ...
    for(let k=0;k<nz;k++){
      const l=cell3DIndex(ge.leftCell,k,nz),r=cell3DIndex(ge.rightCell,k,nz),q=edge3DIndex(e,k,nz),du=acoustic.uEdge[q]!-predictor.uEdge[q]!,uP=predictor.uEdge[q]!,drAvg=.5*((acoustic.rhoD[l]!-predictor.rhoD[l]!)+(acoustic.rhoD[r]!-predictor.rhoD[r]!)),dxAvg=.5*((acoustic.rhoThetaM[l]!-predictor.rhoThetaM[l]!)+(acoustic.rhoThetaM[r]!-predictor.rhoThetaM[r]!)),rhoAvg=.5*(predictor.rhoD[l]!+predictor.rhoD[r]!),xAvg=.5*(predictor.rhoThetaM[l]!+predictor.rhoThetaM[r]!),A=edgeLength*v.dz[k]!,fm=(rhoAvg*du+uP*drAvg)*A,fx=(xAvg*du+uP*dxAvg)*A;
