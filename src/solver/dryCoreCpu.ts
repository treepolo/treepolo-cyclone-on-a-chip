...
export class DryCoreCpu {
  ...
  constructor(public readonly h:CubedSphereGrid,public readonly v:VerticalGrid,public readonly ref:ReferenceAtmosphere,public readonly heviOffCentering=0){}
  step(s:DryState,dt:number):StepDiagnostics{
    ...
    const cr=new Float64Array(nz),cx=new Float64Array(nz),cw=new Float64Array(nz+1);for(let c=0;c<h.cellCount;c++){for(let k=0;k<nz;k++){cr[k]=s.rhoD[cell3DIndex(c,k,nz)]!;cx[k]=s.rhoThetaM[cell3DIndex(c,k,nz)]!}for(let k=0;k<=nz;k++)cw[k]=s.wInterface[w3DIndex(c,k,nz)]!;heviColumnStep(v,ref,{rho:cr,rhoTheta:cx,w:cw},dt,this.heviOffCentering);for(let k=0;k<nz;k++){s.rhoD[cell3DIndex(c,k,nz)]=cr[k]!;s.rhoThetaM[cell3DIndex(c,k,nz)]=cx[k]!}for(let k=0;k<=nz;k++)s.wInterface[w3DIndex(c,k,nz)]=cw[k]!;}
