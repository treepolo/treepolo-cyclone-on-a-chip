...
/**
 * Mass fluxes integrated over their canonical faces (kg/s for rho).
 * vMassFlux is the TOTAL effective vertical mass flux over the full dry-core
 * step: the time-centered HEVI reference flux plus the outer perturbation flux.
 * This makes the snapshot continuity-consistent for Stage-4 momentum transport.
 */
...
export class DryCoreCpu {
  ...
  step(s:DryState,dt:number,heviRayleighEnabled=true):StepDiagnostics{
    ...
    const{h,v,ref}=this,nz=v.nz,R=EARTH.radius,p=new Float64Array(h.cellCount*nz);
    const hMass=this.captureTransport?new Float64Array(s.uEdge.length):undefined;
    const vMass=this.captureTransport?new Float64Array(s.wInterface.length):undefined;
    ...
    for(let c=0;c<h.cellCount;c++){
      ...
      const refFluxPerArea=heviColumnStep(v,ref,{rho:cr,rhoTheta:cx,w:cw},dt,this.heviOffCentering,heviRayleighEnabled?this.heviRayleighRates:undefined);
      if(vMass){const area=h.cellAreaUnit[c]!*R*R;for(let i=0;i<=nz;i++)vMass[w3DIndex(c,i,nz)]=refFluxPerArea[i]!*area;}
    ...
    for(let c=0;c<h.cellCount;c++)for(let i=1;i<nz;i++){const l=cell3DIndex(c,i-1,nz),u=cell3DIndex(c,i,nz),wi=w3DIndex(c,i,nz),rho=.5*(s.rhoD[l]!+s.rhoD[u]!),rho0=.5*(ref.rhoCenter[i-1]!+ref.rhoCenter[i]!);s.wInterface[wi]=s.wInterface[wi]!-dt*EARTH.gravity*(rho-rho0)/Math.max(rho,1e-12)}
