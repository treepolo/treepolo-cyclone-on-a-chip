...
@group(0)@binding(4)var<storage,read>cw:array<vec4<f32>>;
@group(0)@binding(5)var<storage,read_write>mf:array<vec4<f32>>;
@compute @workgroup_size(128)fn main(@builtin(global_invocation_id)gid:vec3<u32>){let q=gid.x;let count=P.cellCount*(P.nz+1u);if(q>=count){return;}let c=q/(P.nz+1u);let i=q-c*(P.nz+1u);if(i==0u||i==P.nz){mf[q]=vec4<f32>(0.0);return;}let vel=w[q];let k=select(i,i-1u,vel>=0.0);let m=rho[c*P.nz+k]*vel*cellArea[c];mf[q]=vec4<f32>(cw[c*P.nz+k].xyz*m,0.0);}
`;
