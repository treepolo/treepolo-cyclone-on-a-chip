...
export class GpuRotatingDryCore{
  ...
  private encodeOne(enc:GPUAny,heldSuarez:boolean):void{
    ...
    {const p=enc.beginComputePass(),seq:[string,number,number][]=[['pressure',this.h.cellCount*this.v.nz,128],['hvel',this.h.edgeCount*this.v.nz,128],['hevi',this.h.cellCount,1],['hflux',this.h.edgeCount*this.v.nz,128],['vflux',this.h.cellCount*(this.v.nz+1),128],['divergence',this.h.cellCount*this.v.nz,128]];for(const[name,count,size]of seq){p.setPipeline(this.core.pipelines[name]);p.setBindGroup(0,this.core.bindGroups[name]);p.dispatchWorkgroups(Math.ceil(count/size))}p.end()}
