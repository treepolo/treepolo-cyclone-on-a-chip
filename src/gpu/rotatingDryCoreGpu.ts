...
@group(0)@binding(1)var<storage,read>vFlux:array<vec2<f32>>;
@group(0)@binding(2)var<storage,read>heviRefMass:array<f32>;
@group(0)@binding(3)var<storage,read>cw:array<vec4<f32>>;
@group(0)@binding(4)var<storage,read_write>mf:array<vec4<f32>>;
@compute @workgroup_size(128)fn main(@builtin(global_invocation_id)gid:vec3<u32>){let q=gid.x;let count=P.cellCount*(P.nz+1u);if(q>=count){return;}let c=q/(P.nz+1u);let i=q-c*(P.nz+1u);if(i==0u||i==P.nz){mf[q]=vec4<f32>(0.0);return;}let m=vFlux[q].x+heviRefMass[q];let k=select(i,i-1u,m>=0.0);mf[q]=vec4<f32>(cw[c*P.nz+k].xyz*m,0.0);}
`;
...
@group(0)@binding(7)var<storage,read_write>wOut:array<f32>;
fn ldz(k:u32)->f32{return layerRef[k*5u+1u];}
@compute @workgroup_size(128)fn main(@builtin(global_invocation_id)gid:vec3<u32>){
  let q=gid.x;let count=P.cellCount*(P.nz+1u);if(q>=count){return;}let c=q/(P.nz+1u);let i=q-c*(P.nz+1u);
...
@group(0)@binding(2)var<storage,read>cellEdges:array<vec4<u32>>;
@group(0)@binding(3)var<storage,read>cellSigns:array<vec4<i32>>;
@group(0)@binding(4)var<storage,read>hFlux:array<vec2<f32>>;
@group(0)@binding(5)var<storage,read>vFlux:array<vec2<f32>>;
@group(0)@binding(6)var<storage,read>heviRefMass:array<f32>;
@group(0)@binding(7)var<storage,read>rho:array<f32>;
@group(0)@binding(8)var<storage,read_write>rhoOld:array<f32>;
@compute @workgroup_size(128)fn main(@builtin(global_invocation_id)gid:vec3<u32>){let q=gid.x;if(q>=P.cellCount*P.nz){return;}let c=q/P.nz;let k=q-c*P.nz;let ee=cellEdges[c];let ss=cellSigns[c];var mt=0.0;for(var s:u32=0u;s<4u;s++){mt-=f32(ss[s])*hFlux[ee[s]*P.nz+k].x;}let vb=c*(P.nz+1u)+k;let vt=vb+1u;mt+=(vFlux[vb].x+heviRefMass[vb])-(vFlux[vt].x+heviRefMass[vt]);rhoOld[q]=rho[q]-P.dt*mt/volume[q];}
`;
...
export class GpuRotatingDryCore{
  ...
  private constructor(public readonly core:GpuDryCorePrototype,public readonly h:CubedSphereGrid,public readonly v:VerticalGrid,public readonly ref:ReferenceAtmosphere){
    ...
    const edgeGeom=new Float32Array(h.edgeCount*4);for(let e=0;e<h.edgeCount;e++){const ge=h.edges[e]!;edgeGeom.set([ge.normal[0],ge.normal[1],ge.normal[2],ge.midpoint[2]],e*4)}
    const cellGeom=new Float32Array(h.cellCount*8);for(let c=0;c<h.cellCount;c++)cellGeom.set([g.east[c*3]!,g.east[c*3+1]!,g.east[c*3+2]!,g.radial[c*3+2]!,g.north[c*3]!,g.north[c*3+1]!,g.north[c*3+2]!,0],c*8);
    const volume=new Float32Array(h.cellCount*v.nz);for(let c=0;c<h.cellCount;c++)for(let k=0;k<v.nz;k++)volume[c*v.nz+k]=h.cellAreaUnit[c]!*EARTH.radius*EARTH.radius*v.dz[k]!;
    const wAdvMeta=new Int32Array(h.cellCount*4*4);for(let c=0;c<h.cellCount;c++)for(let slot=0;slot<4;slot++){const e=h.cellEdges[c*4+slot]!,ge=h.edges[e]!,o=(c*4+slot)*4;wAdvMeta[o]=e;wAdvMeta[o+1]=ge.leftCell===c?ge.rightCell:ge.leftCell;wAdvMeta[o+2]=h.cellEdgeSigns[c*4+slot]!;wAdvMeta[o+3]=0;}
    this.buffers.edgeGeom=makeBuffer(this.device,edgeGeom,storage,'stage4 edge geom');this.buffers.cellGeom=makeBuffer(this.device,cellGeom,storage,'stage4 cell geom');this.buffers.recon=makeBuffer(this.device,Float32Array.from(g.reconstruction),storage,'stage4 reconstruction');this.buffers.volume=makeBuffer(this.device,volume,storage,'stage4 cell volume');this.buffers.wAdvMeta=makeBuffer(this.device,wAdvMeta,storage,'stage4 w advection cell-edge metadata');this.buffers.uTmp=makeEmpty(this.device,core.buffers.u.size??h.edgeCount*v.nz*4,storage,'stage4 u temp');this.buffers.wTmp=makeEmpty(this.device,core.buffers.w.size??h.cellCount*(v.nz+1)*4,storage,'stage4 w temp');this.buffers.cellWind=makeEmpty(this.device,h.cellCount*v.nz*16,storage,'stage4 cell wind/delta');this.buffers.hMom=makeEmpty(this.device,h.edgeCount*v.nz*16,storage,'stage4 h momentum flux');this.buffers.vMom=makeEmpty(this.device,h.cellCount*(v.nz+1)*16,storage,'stage4 v momentum flux');this.buffers.rhoOld=makeEmpty(this.device,h.cellCount*v.nz*4,storage,'stage4 rho before dry transport');this.build();
  }
  static async create(h:CubedSphereGrid,v:VerticalGrid,ref:ReferenceAtmosphere,state:DryState):Promise<GpuRotatingDryCore>{const core=await GpuDryCorePrototype.create(h,v,ref,state,STAGE4_HEVI_OFFCENTERING,buildModelTopSpongeRates(v));core.device.pushErrorScope?.('validation');const out=new GpuRotatingDryCore(core,h,v,ref);const err=await core.device.popErrorScope?.();if(err){out.destroy();throw new Error(`Stage 4 WebGPU validation: ${err.message||err}`)}return out}
  ...
  private encodeOne(enc:GPUAny,heldSuarez:boolean):void{
    ...
    {const p=enc.beginComputePass();this.dispatch(p,'cellWind',this.h.cellCount*this.v.nz);this.dispatch(p,'hMom',this.h.edgeCount*this.v.nz);this.dispatch(p,'vMom',this.h.cellCount*(this.v.nz+1));this.dispatch(p,'rhoOld',this.h.cellCount*this.v.nz);this.dispatch(p,'momDelta',this.h.cellCount*this.v.nz);this.dispatch(p,'project',this.h.edgeCount*this.v.nz);this.dispatch(p,'wAdvect',this.h.cellCount*(this.v.nz+1));p.end()}
    enc.copyBufferToBuffer(this.buffers.uTmp,0,this.core.buffers.u,0,this.h.edgeCount*this.v.nz*4);enc.copyBufferToBuffer(this.buffers.wTmp,0,this.core.buffers.w,0,this.h.cellCount*(this.v.nz+1)*4);
    if(heldSuarez){const p=enc.beginComputePass();this.dispatch(p,'thermal',this.h.cellCount*this.v.nz);this.dispatch(p,'drag',this.h.edgeCount*this.v.nz);p.end()}this.onePass(enc,'coriolis',this.h.edgeCount*this.v.nz);enc.copyBufferToBuffer(this.buffers.uTmp,0,this.core.buffers.u,0,this.h.edgeCount*this.v.nz*4);
  }
