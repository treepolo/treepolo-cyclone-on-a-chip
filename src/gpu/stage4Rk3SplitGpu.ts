...
@compute @workgroup_size(${STAGE4_VERTICAL_WORKGROUP_SIZE})fn main(@builtin(global_invocation_id)gid:vec3<u32>){let c=gid.x;if(c>=P.cellCount||P.nz>MAX_NZ||P.nz<2u){return;}let nz=P.nz;let n=nz-1u;let theta=.5*(1.0+clamp(P.offCenter,0.0,.999));let oldWeight=1.0-theta;var drOld:array<f32,128>;var dxOld:array<f32,128>;var dwOld:array<f32,129>;var pPred:array<f32,128>;var dpdX:array<f32,128>;var drBase:array<f32,128>;var dxBase:array<f32,128>;var lOld:array<f32,129>;var lo:array<f32,127>;var di:array<f32,127>;var up:array<f32,127>;var rhs:array<f32,127>;var cp:array<f32,127>;var dp:array<f32,127>;var sol:array<f32,127>;var dwNew:array<f32,129>;let bc=c*nz;let bw=c*(nz+1u);for(var k:u32=0u;k<nz;k++){let pc=predCell[bc+k];let ac=acousticCell[bc+k];drOld[k]=ac.x-pc.x;dxOld[k]=ac.y-pc.y;pPred[k]=pressure(pc.y);dpdX[k]=P.gamma*pPred[k]/max(pc.y,1e-12);}for(var i:u32=0u;i<=nz;i++){let f=iface[bw+i];dwOld[i]=f.y-f.x;dwNew[i]=0.0;}for(var k:u32=0u;k<nz;k++){let rr=frozenScalar[bc+k]+hCorr[bc+k];let divR=(irho(k+1u)*dwOld[k+1u]-irho(k)*dwOld[k])/ldz(k);let divX=(ix(k+1u)*dwOld[k+1u]-ix(k)*dwOld[k])/ldz(k);drBase[k]=drOld[k]+P.dt*rr.x-P.dt*oldWeight*divR;dxBase[k]=dxOld[k]+P.dt*rr.y-P.dt*oldWeight*divX;}for(var i:u32=1u;i<nz;i++){let l=i-1u;let u=i;let cl=predCell[bc+l];let cu=predCell[bc+u];let rr=.5*(cl.x+cu.x);let rr0=.5*(lrho(l)+lrho(u));let den=max(.5*(rr0+rr),1e-12);let dzc=lz(u)-lz(l);let dpPrime=(pPred[u]-lp(u))-(pPred[l]-lp(l));let cr=dpPrime*.25/(den*den*dzc)-P.gravity*rr0*.5/max(rr*rr,1e-24);let cxl=dpdX[l]/(den*dzc);let cxu=-dpdX[u]/(den*dzc);lOld[i]=cr*(drOld[l]+drOld[u])+cxl*dxOld[l]+cxu*dxOld[u];}for(var ii:u32=0u;ii<n;ii++){let i=ii+1u;let l=i-1u;let u=i;let cl=predCell[bc+l];let cu=predCell[bc+u];let rr=.5*(cl.x+cu.x);let rr0=.5*(lrho(l)+lrho(u));let den=max(.5*(rr0+rr),1e-12);let dzc=lz(u)-lz(l);let dpPrime=(pPred[u]-lp(u))-(pPred[l]-lp(l));let cr=dpPrime*.25/(den*den*dzc)-P.gravity*rr0*.5/max(rr*rr,1e-24);let cxl=dpdX[l]/(den*dzc);let cxu=-dpdX[u]/(den*dzc);let l0=cr*(drBase[l]+drBase[u])+cxl*dxBase[l]+cxu*dxBase[u];let alm=cr*(P.dt*theta*irho(i-1u)/ldz(l))+cxl*(P.dt*theta*ix(i-1u)/ldz(l));let ali=cr*(-P.dt*theta*irho(i)/ldz(l)+P.dt*theta*irho(i)/ldz(u))+cxl*(-P.dt*theta*ix(i)/ldz(l))+cxu*(P.dt*theta*ix(i)/ldz(u));let alp=cr*(-P.dt*theta*irho(i+1u)/ldz(u))+cxu*(-P.dt*theta*ix(i+1u)/ldz(u));let f=iface[bw+i];let rate=max(0.0,f.w);lo[ii]=-P.dt*theta*alm;di[ii]=1.0+P.dt*rate-P.dt*theta*ali;up[ii]=-P.dt*theta*alp;rhs[ii]=dwOld[i]+P.dt*f.z+P.dt*oldWeight*lOld[i]+P.dt*theta*l0-P.dt*rate*f.x;}lo[0u]=0.0;up[n-1u]=0.0;var denom=di[0u];cp[0u]=select(up[0u]/denom,0.0,n==1u);dp[0u]=rhs[0u]/denom;for(var j:u32=1u;j<n;j++){denom=di[j]-lo[j]*cp[j-1u];cp[j]=select(up[j]/denom,0.0,j==n-1u);dp[j]=(rhs[j]-lo[j]*dp[j-1u])/denom;}sol[n-1u]=dp[n-1u];var jj:i32=i32(n)-2;loop{if(jj<0){break;}let j=u32(jj);sol[j]=dp[j]-cp[j]*sol[j+1u];jj=jj-1;}for(var i:u32=1u;i<nz;i++){dwNew[i]=sol[i-1u];}for(var k:u32=0u;k<nz;k++){let dr=drBase[k]-P.dt*theta*(irho(k+1u)*dwNew[k+1u]-irho(k)*dwNew[k])/ldz(k);let dx=dxBase[k]-P.dt*theta*(ix(k+1u)*dwNew[k+1u]-ix(k)*dwNew[k])/ldz(k);acousticCell[bc+k]=predCell[bc+k]+vec2<f32>(dr,dx);}for(var i:u32=0u;i<=nz;i++){var f=iface[bw+i];let nw=select(f.x+dwNew[i],0.0,i==0u||i==nz);let weighted=f.x+oldWeight*dwOld[i]+theta*dwNew[i];f.y=nw;iface[bw+i]=f;refFlux[bw+i]=vec2<f32>(irho(i)*weighted,ix(i)*weighted);}}
`;
