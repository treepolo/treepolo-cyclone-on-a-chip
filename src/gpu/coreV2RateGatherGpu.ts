...
export interface CoreV2GpuRateStaticData {
  ...
  /** vec4 per horizontal cell: bottom face dx xyz, p_ref. */
  bottomDxReferencePressure:Float32Array;
  /** vec4 per horizontal cell: bottom outward vector area xyz. */
  bottomOutwardArea:Float32Array;
  /** vec4 per horizontal cell: top face dx xyz, p_ref. */
  topDxReferencePressure:Float32Array;
  /** vec4 per horizontal cell: top outward vector area xyz. */
  topOutwardArea:Float32Array;
...
export function buildCoreV2GpuRateStaticData(
  ...
  const bottomDxReferencePressure=new Float32Array(hc*4),bottomOutwardArea=new Float32Array(hc*4),topDxReferencePressure=new Float32Array(hc*4),topOutwardArea=new Float32Array(hc*4);
  for(let c=0;c<hc;c++){
    ...
    const ab=radialFaceVectorArea(geometry,c,0),at=radialFaceVectorArea(geometry,c,nz);const i=c*4;
    bottomDxReferencePressure[i]=fb[0]-xb[0];bottomDxReferencePressure[i+1]=fb[1]-xb[1];bottomDxReferencePressure[i+2]=fb[2]-xb[2];bottomDxReferencePressure[i+3]=reference.radialFacePressure[0]!;
    bottomOutwardArea[i]=-ab[0];bottomOutwardArea[i+1]=-ab[1];bottomOutwardArea[i+2]=-ab[2];
    topDxReferencePressure[i]=ft[0]-xt[0];topDxReferencePressure[i+1]=ft[1]-xt[1];topDxReferencePressure[i+2]=ft[2]-xt[2];topDxReferencePressure[i+3]=reference.radialFacePressure[nz]!;
    topOutwardArea[i]=at[0];topOutwardArea[i+1]=at[1];topOutwardArea[i+2]=at[2];
  }
  return {cellCount,horizontalCellCount:hc,nz,cellFacePairs:pairs,gravityCoefficient,bottomDxReferencePressure,bottomOutwardArea,topDxReferencePressure,topOutwardArea,referenceLayer};
}

const SHADER=/* wgsl */`
struct Params{cellCount:u32,nz:u32,_a:u32,_b:u32,gamma:f32,_pad:vec3<f32>};
...
@group(0)@binding(6)var<storage,read>bottomDxRef:array<vec4<f32>>;
@group(0)@binding(7)var<storage,read>bottomArea:array<vec4<f32>>;
@group(0)@binding(8)var<storage,read>topDxRef:array<vec4<f32>>;
...
@compute @workgroup_size(128)fn main(@builtin(global_invocation_id)gid:vec3<u32>){
 ...
 if(k==0u){let d=bottomDxRef[c];let pp=pprime(q)+dot(gradient[q*5u+4u].xyz,d.xyz);a.yzw-=pp*bottomArea[c].xyz;}
 if(k+1u==P.nz){let d=topDxRef[c];let pp=pprime(q)+dot(gradient[q*5u+4u].xyz,d.xyz);a.yzw-=pp*topArea[c].xyz;}
