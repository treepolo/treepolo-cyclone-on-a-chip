 ...
 * specialized to this spherical cubed-sphere topology. Diagnostic only.
 */
function applySymmetricH(h:CubedSphereGrid,V:Float64Array):Float64Array{
  ...
  for(let c=0;c<h.cellCount;c++)for(let s=0;s<4;s++){
    ...
    // Appendix A uses s_c=4 for a quadrilateral dual cell and s_c=6 for a
    // triangular dual cell.  The eight cube-corner vertices are triangular;
    // using the raw valence (=3) would double their kinetic-energy weight.
