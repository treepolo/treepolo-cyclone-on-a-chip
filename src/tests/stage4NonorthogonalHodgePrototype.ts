...
/** Exact line integral of u=AMP*(AXIS x r) along the oriented great-circle dual edge left->right. */
...
/** Exact integral of the same solid-body wind through the oriented primal face. */
...
function applySymmetricH(h:CubedSphereGrid,V:Float64Array,vertexCells=buildVertexCells(h)):Float64Array{
  ...
  for(let c=0;c<h.cellCount;c++)for(let s=0;s<4;s++){
    ...
    const sc=valence===4?4:6,d0=dualVectorAtCell(h,e0,c),d1=dualVectorAtCell(h,e1,c),area=Math.hypot(...cross3(d0,d1));
...
function run(n:number){
  const h=buildCubedSphere(n),vertexCells=buildVertexCells(h),V=new Float64Array(h.edgeCount),target=new Float64Array(h.edgeCount);for(let e=0;e<h.edgeCount;e++){V[e]=dualCirculation(h,e);target[e]=exactFaceFlux(h,e);}
  ...
  for(let e=0;e<h.edgeCount;e++){const ge=h.edges[e]!,d=U[e]!-target[e]!,r=target[e]!,isSeam=h.cellPanel[ge.leftCell]!==h.cellPanel[ge.rightCell],isCorner=(vertexCells.get(pkey(ge.p0))?.size===3)||(vertexCells.get(pkey(ge.p1))?.size===3);add(all,d,r);if(isSeam)add(seam,d,r);else add(interior,d,r);if(isCorner)add(corner,d,r);else if(isSeam)add(seamRegular,d,r);energy+=.5*V[e]!*U[e]!;}
  const A=new Float64Array(h.edgeCount),B=new Float64Array(h.edgeCount);for(let e=0;e<h.edgeCount;e++){A[e]=Math.sin(.71*(e+1));B[e]=Math.cos(.37*(e+2));}const HA=applySymmetricH(h,A,vertexCells),HB=applySymmetricH(h,B,vertexCells);let aHb=0,bHa=0,norm=0;for(let e=0;e<h.edgeCount;e++){aHb+=A[e]!*HB[e]!;bHa+=B[e]!*HA[e]!;norm+=Math.abs(A[e]!*HB[e]!)+Math.abs(B[e]!*HA[e]!);}
...
try{
  ...
  console.log('Stage4 nonorthogonal symmetric Hodge prototype: exact solid-body dual circulation V -> exact primal normal flux U');
  ...
  console.log(`all refine=${ratio(rows[1]!.relL2,rows[2]!.relL2).toFixed(3)},${ratio(rows[2]!.relL2,rows[3]!.relL2).toFixed(3)} interior=${ratio(rows[1]!.interior,rows[2]!.interior).toFixed(3)},${ratio(rows[2]!.interior,rows[3]!.interior).toFixed(3)} regular-seam=${ratio(rows[1]!.seamRegular,rows[2]!.seamRegular).toFixed(3)},${ratio(rows[2]!.seamRegular,rows[3]!.seamRegular).toFixed(3)}`);
  if(rows.some(r=>![r.relL2,r.interior,r.seam,r.seamRegular,r.corner,r.maxOverRms,r.energy,r.symmetry].every(Number.isFinite)))throw new Error('non-finite Hodge result');if(rows.some(r=>!(r.energy>0)))throw new Error('symmetric H quadratic energy is not positive for test wind');if(rows.some(r=>r.symmetry>5e-13))throw new Error(`assembled H is not symmetric: ${rows.map(r=>r.symmetry).join(',')}`);
