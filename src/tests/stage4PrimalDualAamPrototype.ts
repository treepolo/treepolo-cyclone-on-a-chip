...
import { applyNonorthogonalHodge,buildNonorthogonalHodgeStencil,hodgeSymmetryDefect } from '../grid/nonorthogonalHodge.js';
...
/**
 * Globally smooth tangent field with a deliberately non-trivial l=2,m=0
 * divergent component. The earlier l=1 potential was orthogonal to the
 * axisymmetric planetary-AAM weight and made the global P+C test degenerate.
 * Here chi = 8 P2(z) + 5 xz + 3 xy (coefficients in m/s on the unit sphere),
 * so grad_s chi couples directly to cos^2(latitude). A tilted solid-body part
 * keeps the field fully 2-D without changing that purpose.
 */
function wind(r:Vec3):Vec3{
  const [x,y,z]=r,g:Vec3=[5*z+3*y,3*x,24*z+5*x],gd=dot3(g,r),rot=cross3(AXIS,r);
  return[g[0]-gd*x+ROT*rot[0],g[1]-gd*y+ROT*rot[1],g[2]-gd*z+ROT*rot[2]];
}
...
function planetaryMassTorque(h:CubedSphereGrid,U:ArrayLike<number>):number{let torque=0;for(let c=0;c<h.cellCount;c++){let flux=0;for(let s=0;s<4;s++)flux+=h.cellEdgeSigns[c*4+s]!*U[h.cellEdges[c*4+s]!]!;const r=center(h,c),cos2=Math.max(0,1-r[2]*r[2]),specific=EARTH.omega*EARTH.radius*EARTH.radius*cos2;torque+=-flux*specific;}return torque;}
...
interface Metrics{exactPair:number;hodgePair:number;planetExact:number;planetH:number;cor:number;fluxRel:number;sym:number;}
function evaluate(h:CubedSphereGrid):Metrics{const V=analyticV(h),dV=analyticDV(h),Uexact=exactFaceU(h),H=buildNonorthogonalHodgeStencil(h),Uh=applyNonorthogonalHodge(H,V),rec=buildDualReconstruction(h),p0=planetaryMassTorque(h,Uexact),ph=planetaryMassTorque(h,Uh),c=relativeCoriolisTorque(h,rec,dV),scale=86400/leverMass(h);let e2=0,r2=0;for(let e=0;e<h.edgeCount;e++){const d=Uh[e]!-Uexact[e]!;e2+=d*d;r2+=Uexact[e]!*Uexact[e]!;}return{exactPair:(p0+c)*scale,hodgePair:(ph+c)*scale,planetExact:p0*scale,planetH:ph*scale,cor:c*scale,fluxRel:Math.sqrt(e2/Math.max(r2,1e-60)),sym:hodgeSymmetryDefect(H)};}
function run(n:number){return{n,raw:evaluate(buildCubedSphere(n)),mod:evaluate(buildModifiedCubedSphere(n))};}
try{
  const rows=[4,8,16,32].map(run),ratio=(a:number,b:number)=>Math.abs(a)/Math.max(Math.abs(b),1e-30);
  console.log('Stage4 primal-dual AAM prototype; nontrivial l=2 divergence, analytic V/Coriolis integrals, continuity U=HV');
  console.log('All torque columns equivalent m/s/day; exactPair uses exact face U, hodgePair uses H(V).');
  console.log('N\traw Pexact\traw Cor\traw exactPair\traw Hpair\tmod Pexact\tmod PH\tmod Cor\tmod exactPair\tmod Hpair\traw HfluxL2\tmod HfluxL2');
  for(const r of rows)console.log(`${r.n}\t${r.raw.planetExact.toExponential(7)}\t${r.raw.cor.toExponential(7)}\t${r.raw.exactPair.toExponential(7)}\t${r.raw.hodgePair.toExponential(7)}\t${r.mod.planetExact.toExponential(7)}\t${r.mod.planetH.toExponential(7)}\t${r.mod.cor.toExponential(7)}\t${r.mod.exactPair.toExponential(7)}\t${r.mod.hodgePair.toExponential(7)}\t${r.raw.fluxRel.toExponential(6)}\t${r.mod.fluxRel.toExponential(6)}`);
  console.log(`modified H-pair refine=${ratio(rows[1]!.mod.hodgePair,rows[2]!.mod.hodgePair).toFixed(3)},${ratio(rows[2]!.mod.hodgePair,rows[3]!.mod.hodgePair).toFixed(3)} exact-pair=${ratio(rows[1]!.mod.exactPair,rows[2]!.mod.exactPair).toFixed(3)},${ratio(rows[2]!.mod.exactPair,rows[3]!.mod.exactPair).toFixed(3)}`);
  const n8=rows[1]!;if(Math.abs(n8.mod.planetExact)<1e-3||Math.abs(n8.mod.cor)<1e-3)throw new Error(`manufactured P+C test is still trivial at N8: P=${n8.mod.planetExact}, C=${n8.mod.cor}`);
  if(rows.some(r=>![r.raw.exactPair,r.raw.hodgePair,r.mod.exactPair,r.mod.hodgePair,r.raw.fluxRel,r.mod.fluxRel,r.mod.sym].every(Number.isFinite)))throw new Error('non-finite primal-dual AAM result');if(rows.some(r=>r.mod.sym>5e-13))throw new Error('modified H lost symmetry');
}catch(e){console.error('FAIL Stage4 primal-dual AAM prototype');console.error(e);process.exitCode=1;}
