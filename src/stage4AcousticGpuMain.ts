...
run.onclick=()=>void(async()=>{
  ...
  for(const id of ['rho','x','w','cpuW','gpuW','restW','restRho','elapsed'])set(id,'—');
  try{
    const r=await runStage4AcousticGpuAgreement();
    set('rho',r.rhoRelativeL2.toExponential(3));set('x',r.rhoThetaRelativeL2.toExponential(3));set('w',`${r.maxDeltaW.toExponential(3)} m/s`);set('cpuW',`${r.cpuMaxW.toExponential(3)} m/s`);set('gpuW',`${r.gpuMaxW.toExponential(3)} m/s`);set('restW',`${r.hydrostaticMaxW.toExponential(3)} m/s`);set('restRho',r.hydrostaticRhoRelativeL2.toExponential(3));set('elapsed',`${(r.elapsedMs/1000).toFixed(2)} s`);
